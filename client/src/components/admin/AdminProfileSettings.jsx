...
const AdminProfileSettings = ({ openAccordion, toggleAccordion, oldPassword, setOldPassword, newPassword, setNewPassword, handleChangePassword, isLoading, delPassword, setDelPassword, handleDeleteAccount }) => (
  <div className="mt-12 space-y-6 max-w-4xl mx-auto pb-6">
    {/* Change Password */}
    <div className="border rounded-lg bg-white shadow p-6">
      <button className="w-full text-left text-xl font-semibold text-gray-800 flex justify-between items-center" onClick={() => toggleAccordion("changePassword")}>
        Change Password <span>{openAccordion === "changePassword" ? "-" : "+"}</span>
      </button>
      {openAccordion === "changePassword" && (
        <div className="mt-4 space-y-4">
          ...
          <div className="flex space-x-4">
            <button onClick={handleChangePassword} disabled={isLoading} className="bg-primary text-white px-4 py-2 rounded hover:bg-green-700 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
              {isLoading ? "Changing..." : "Change Password"}
            </button>
            <button onClick={() => toggleAccordion("changePassword")} disabled={isLoading} className="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
              Cancel
            </button>
          </div>
        </div>
    ...
    <div className="border rounded-lg bg-white shadow p-6">
      <button className="w-full text-left text-xl font-semibold text-gray-800 flex justify-between items-center" onClick={() => toggleAccordion("deleteAccount")}>
        Delete Account <span>{openAccordion === "deleteAccount" ? "-" : "+"}</span>
      </button>
      {openAccordion === "deleteAccount" && (
        <div className="mt-4 space-y-4">
          ...
          <div className="flex space-x-4">
            <button onClick={handleDeleteAccount} disabled={isLoading} className="bg-primary text-white px-4 py-2 rounded hover:bg-red-700 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
              {isLoading ? "Deleting..." : "Delete Account"}
            </button>
            <button onClick={() => toggleAccordion("deleteAccount")} disabled={isLoading} className="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
              Cancel
            </button>
          </div>
        </div>
